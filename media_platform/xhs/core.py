...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            if config.ENABLE_CDP_MODE:
                utils.logger.info(
                    "[XiaoHongShuCrawler] Launching browser using CDP mode"
                )
                ...
                utils.logger.info(
                    "[XiaoHongShuCrawler] Launching browser using standard mode"
                )
            ...
            if config.CRAWLER_TYPE == "search":
                ...
                # Get hot content (trending/popular content)
                await self.get_hot_content()
            else:
                pass
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (
                ...
                try:
                    ...
                    utils.logger.info(
                        f"[XiaoHongShuCrawler.search] Search notes response: {notes_res}"
                    )
                    if not notes_res or not notes_res.get("has_more", False):
                        utils.logger.info(
                            "[XiaoHongShuCrawler.search] No more content!"
                        )
                        break
    ...
    async def get_creators_and_notes(self) -> None:
        ...
        for creator_url in config.XHS_CREATOR_ID_LIST:
            try:
                ...
                utils.logger.info(
                    f"[XiaoHongShuCrawler.get_creators_and_notes] Parse creator URL info: {creator_info}"
                )
                ...
                utils.logger.error(
                    f"[XiaoHongShuCrawler.get_creators_and_notes] Failed to parse creator URL: {e}"
                )
                continue
        ...
        async with semaphore:
            try:
                ...
                if not note_detail:
                    ...
                    if not note_detail:
                        raise Exception(
                            f"[get_note_detail_async_task] Failed to get note detail, Id: {note_id}"
                        )
                ...
                utils.logger.warning(
                    f"[XiaoHongShuCrawler.get_note_detail_async_task] Note not found: {note_id}, {ex}"
                )
                return None
    ...
    async def launch_browser_with_cdp(
        ...
        try:
            ...
            utils.logger.error(
                f"[XiaoHongShuCrawler] CDP mode launch failed, falling back to standard mode: {e}"
            )
    ...
    async def get_hot_content(self) -> None:
        """
        Get hot/trending content from Xiaohongshu.
        If keywords are specified, search for those keywords as hot topics.
        If no keywords specified, try to get platform's trending content.

        New logic:
        1. Search for notes based on keywords or default "热门"
        2. Collect all note details
        3. Filter notes from the last 3 days
        4. Sort by interaction score (likes + collects + comments + shares)
        5. Output top 50 hottest notes
        """
        import datetime
        import time

        utils.logger.info(
            "[XiaoHongShuCrawler.get_hot_content] Begin to get hot content from Xiaohongshu with new ranking logic"
        )

        # Check if keywords are specified
        if config.KEYWORDS and config.KEYWORDS.strip():
            # If keywords are specified, treat them as hot topics to search
            utils.logger.info(
                f"[XiaoHongShuCrawler.get_hot_content] Using specified keywords as hot topics: {config.KEYWORDS}"
            )
            keywords_list = config.KEYWORDS.split(",")
        else:
            # If no keywords specified, use default hot keyword
            utils.logger.info(
                "[XiaoHongShuCrawler.get_hot_content] No keywords specified, using default hot search"
            )
        ...
        # Similar to search mode but with different logging
        xhs_limit_count = 20  # Xiaohongshu limit page fixed value
        if config.CRAWLER_MAX_NOTES_COUNT < xhs_limit_count:
            config.CRAWLER_MAX_NOTES_COUNT = xhs_limit_count
        start_page = config.START_PAGE
        ...
        for keyword in keywords_list:
            source_keyword_var.set(keyword)
            utils.logger.info(
                f"[XiaoHongShuCrawler.get_hot_content] Searching hot topic: {keyword}"
            )
            page = 1
            search_id = get_search_id()
            ...
            while (
                ...
                if page < start_page:
                    utils.logger.info(
                        f"[XiaoHongShuCrawler.get_hot_content] Skip page {page}"
                    )
                    page += 1
                    continue
                ...
                try:
                    utils.logger.info(
                        f"[XiaoHongShuCrawler.get_hot_content] search Xiaohongshu hot topic: {keyword}, page: {page}"
                    )
                    note_ids: List[str] = []
                    xsec_tokens: List[str] = []
                    notes_res = await self.xhs_client.get_note_by_keyword(
                    ...
                    if not notes_res or not notes_res.get("has_more", False):
                        utils.logger.info(
                            "[XiaoHongShuCrawler.get_hot_content] No more content!"
                        )
                        break
                    ...
                    for note_detail in note_details:
                        if note_detail:
                            ...
                            if note_time and note_time >= days_ago:
                                ...
                                collected_count = int(
                                    note_detail.get("collected_count", 0) or 0
                                )
                                comment_count = int(
                                    note_detail.get("comment_count", 0) or 0
                                )
                                share_count = int(
                                    note_detail.get("share_count", 0) or 0
                                )

                                # Interaction score formula (weighted)
                                interaction_score = (
                                    liked_count * 1.0
                                    + collected_count * 1.5  # 收藏权重更高
                                    + comment_count * 2.0  # 评论权重最高
                                    + share_count * 1.2  # 分享权重中等
                                )

                                note_detail["interaction_score"] = interaction_score
                                note_detail["total_interactions"] = (
                                    liked_count
                                    + collected_count
                                    + comment_count
                                    + share_count
                                ...
                                all_note_details.append(note_detail)

                                # Store note for later use if needed
                                await xhs_store.update_xhs_note(note_detail)
                                await self.get_notice_media(note_detail)
                                ...
                                utils.logger.info(
                                    f"[XiaoHongShuCrawler.get_hot_content] Skipping old note from {datetime.datetime.fromtimestamp(note_time or 0).strftime('%Y-%m-%d') if note_time else 'unknown date'}"
                                )
                    ...
                    utils.logger.error(
                        f"[XiaoHongShuCrawler.get_hot_content] search hot topic: {keyword} error: {e}"
                    )
                    break

        # Sort notes by interaction score (descending)
        all_note_details.sort(key=lambda x: x.get("interaction_score", 0), reverse=True)

        # Take top 50 notes
        top_notes = all_note_details[:50]
        ...
        for i, note in enumerate(top_notes, 1):
            ...
            score = note.get("interaction_score", 0)
            ...
            utils.logger.info(
                f"[XiaoHongShuCrawler.get_hot_content] #{i}: ID={note_id}, "
                f"Title='{title}...', Score={score:.1f}, "
                f"Likes={likes}, Collects={collects}, Comments={comments}, Shares={shares}"
            )

        utils.logger.info(
            "[XiaoHongShuCrawler.get_hot_content] Hot content crawling finished"
        )
