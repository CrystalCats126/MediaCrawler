...
class DouYinCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            if config.CRAWLER_TYPE == "search":
                ...
                # Get hot/trending content from Douyin
        ...
        for video_url in config.DY_SPECIFIED_ID_LIST:
            try:
                ...
                if video_info.url_type == "short":
                    utils.logger.info(
                        f"[DouYinCrawler.get_specified_awemes] Resolving short link: {video_url}"
                    )
                    ...
                    if resolved_url:
                        ...
                        utils.logger.info(
                            f"[DouYinCrawler.get_specified_awemes] Short link resolved to aweme ID: {video_info.aweme_id}"
                        )
                    else:
                        utils.logger.error(
                            f"[DouYinCrawler.get_specified_awemes] Failed to resolve short link: {video_url}"
                        )
                        continue

                aweme_id_list.append(video_info.aweme_id)
                utils.logger.info(
                    f"[DouYinCrawler.get_specified_awemes] Parsed aweme ID: {video_info.aweme_id} from {video_url}"
                )
            except ValueError as e:
                utils.logger.error(
                    f"[DouYinCrawler.get_specified_awemes] Failed to parse video URL: {e}"
                )
                continue
    ...
    async def get_aweme_detail(
        ...
        async with semaphore:
            try:
                ...
                await asyncio.sleep(config.CRAWLER_MAX_SLEEP_SEC)
                utils.logger.info(
                    f"[DouYinCrawler.get_aweme_detail] Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after fetching aweme {aweme_id}"
                )
                return result
    ...
    async def get_comments(self, aweme_id: str, semaphore: asyncio.Semaphore) -> None:
        async with semaphore:
            try:
                ...
                utils.logger.info(
                    f"[DouYinCrawler.get_comments] Sleeping for {crawl_interval} seconds after fetching comments for aweme {aweme_id}"
                )
                utils.logger.info(
    ...
    async def get_creators_and_videos(self) -> None:
        ...
        utils.logger.info(
            "[DouYinCrawler.get_creators_and_videos] Parsing creator URLs..."
        )
        ...
        for creator_url in config.DY_CREATOR_ID_LIST:
            try:
                ...
                utils.logger.info(
                    f"[DouYinCrawler.get_creators_and_videos] Parsed sec_user_id: {user_id} from {creator_url}"
                )
            except ValueError as e:
                utils.logger.error(
                    f"[DouYinCrawler.get_creators_and_videos] Failed to parse creator URL: {e}"
                )
                continue
    ...
    async def get_aweme_media(self, aweme_item: Dict):
        ...
        if not config.ENABLE_GET_MEIDAS:
            utils.logger.info(
                f"[DouYinCrawler.get_aweme_media] Crawling image mode is not enabled"
            )
            return
    ...
    async def get_hot_content(self) -> None:
        ...
        """
        utils.logger.info(
            "[DouYinCrawler.get_hot_content] Begin to get hot content from Douyin"
        )
        ...
        if config.KEYWORDS and config.KEYWORDS.strip():
            ...
            utils.logger.info(
                f"[DouYinCrawler.get_hot_content] Using specified keywords as hot topics: {config.KEYWORDS}"
            )
            keywords_list = config.KEYWORDS.split(",")
        else:
            # If no keywords specified, use default hot keyword
            utils.logger.info(
                "[DouYinCrawler.get_hot_content] No keywords specified, using default hot search"
            )
        ...
        for keyword in keywords_list:
            source_keyword_var.set(keyword)
            utils.logger.info(
                f"[DouYinCrawler.get_hot_content] Searching hot topic: {keyword}"
            )
            aweme_list: List[str] = []
            page = 0
            dy_search_id = ""
            ...
            while (
                ...
                if page < start_page:
                    utils.logger.info(f"[DouYinCrawler.get_hot_content] Skip {page}")
                    page += 1
                    continue

                try:
                    utils.logger.info(
                        f"[DouYinCrawler.get_hot_content] search douyin hot topic: {keyword}, page: {page}"
                    )
                    posts_res = await self.dy_client.search_info_by_keyword(
                        keyword=keyword,
                        offset=page * dy_limit_count - dy_limit_count,
                    ...
                    if posts_res.get("data") is None or posts_res.get("data") == []:
                        utils.logger.info(
                            f"[DouYinCrawler.get_hot_content] search douyin hot topic: {keyword}, page: {page} is empty"
                        )
                        break

                    if "data" not in posts_res:
                        utils.logger.error(
                            f"[DouYinCrawler.get_hot_content] search douyin hot topic: {keyword} failed，账号也许被风控了。"
                        )
                        break
                    ...
                    utils.logger.error(
                        f"[DouYinCrawler.get_hot_content] search hot topic: {keyword} error: {e}"
                    )
                    break

        utils.logger.info(
            "[DouYinCrawler.get_hot_content] Hot content crawling finished"
        )
